...
public class Transaction : BaseAuditable
{
        ...
        string? attachmentUrl)
    {
        ...
        if (isTransfer && transferId is null) throw new TransferIdRequiredException();

        if (!isTransfer && transferId is not null) throw new TransferIdForbiddenException();
    ...
    {
        if (TransactionType is TransactionTypeEnum.TransferIn or TransactionTypeEnum.TransferOut) throw new TransferPartialModificationException();
